...
def caculate_feature(all_text, all_text_like, all_text_retweet):
    vec = {
        ...
        "times_like":cf.times_like(all_text_like),
        "times_retweeted":cf.times_retweeted(all_text_retweet)
    }
    return vec
...
if __name__ == '__main__':
    ...
    the_header = ["id","characters", "hashtags", "words", "hashtags_to_words", "mentions",
                  "numeric_chacacters", "symbols", "urls", "urls_to_words", "times_like", "times_retweeted"]
        ...
        for item in tweets_list:
            for k in range(0, flag_lens, 2):
                if item>= flag[k] and item<=flag[k+1]:
                    ...
                    like.append(msg['like'])
                    retweet.append(msg['retweet'])
                    break
